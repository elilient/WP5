...
class HMW_Controllers_Notice extends HMW_Classes_FrontController {
    ...
    public function hmw_admin_notices($notices) {
        ...
        $ignore = add_query_arg(array('hmw_admin_notice' => 'two_week_review', 'hmw_admin_notice_action' => 'later'), admin_url());
        $disable = add_query_arg(array('hmw_admin_notice' => 'two_week_review', 'hmw_admin_notice_action' => 'disable'), admin_url(), admin_url());
        if (is_string($disable) && $disable <> '') {
            $notices['two_week_review'] = array(
                'title' => __('Thank you for using Hide My WP?', _HMW_PLUGIN_NAME_),
                'msg' => sprintf(__("Want to use all %sHide My WP features%s? Get Hide My WP Ghost today with just %s$10/website%s if you buy a 5 Websites License pack", _HMW_PLUGIN_NAME_), '<strong style="color: red">', '</strong>', '<strong style="color: red">', '</strong>'),
                'link' => '<li><span class="dashicons dashicons-external"></span><a href="https://hidemywpghost.com/buy/5_websites_special" target="_blank" style="font-weight: bold">' . __("Go To Offer", _HMW_PLUGIN_NAME_) . '</a></li>',
